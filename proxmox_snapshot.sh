...
cleanup_snapshots() {
    ...
    echo "$snapshots_json" | jq -r '.[] | select(.name | startswith("'"$BASE_SNAPSHOT_NAME"'-")) | [.name, .snaptime] | @tsv' | while IFS=$'\t' read -r snap_name snap_time; do
        ...
        if [ $age_hours -gt $SNAPSHOT_RETENTION_HOURS ]; then
            ...
            # Adjust week_diff for year boundary crossing
            if [ $(date -d "@$snap_time" +%Y) -lt $(date +%Y) ]; then
                local weeks_in_year=$(date -d "$(date +%Y-12-31)" +%V)
                week_diff=$((week_diff + weeks_in_year))
            fi
